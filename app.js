...
app.use((req, res, next) => {
  ...
  if (
      ...
      if (req.session && req.session.user) {
        ...
        if (!req.session.user.TTL || req.session.user.TTL - now < 0) {
          ...
          api.get('/v1/user/profile?id=' + req.session.user._id, doWithUserProfile);
  ...
  function doWithUserProfile(err, response, body) {
    ...
    api.get('/v1/outlet/purchases?shallow=true&id=' + req.session.user.outlet._id, doWithPurchases);

  }

  function doWithPurchases(err, response, body) {
