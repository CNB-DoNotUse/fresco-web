...
var User = {
	...
	refresh: (req, res, next) => {
		var options = {
			  url: '/user/profile?id=' + req.session.user._id,
		...
		API.request(options, (err, response) => {
			...
			if (response.body.err || response.body.error || !response.body.data._id) {
			...
			API.request({
				url: '/v1/outlet/purchases?shallow=true&id=' + req.session.user.outlet._id,
				method: 'GET'
			}, (err, response) => {
