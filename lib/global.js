...
module.exports = {
    ...
    baseUrl: function() {
        if(typeof(window.mode) !== 'undefined') {
            if(window.mode == 'dev')
                return 'http://dev.fresconews.com';
            else if(window.mode == 'staging')
                return 'https://staging.fresconews.com';

            return 'https://fresconews.com';
        }
    ...
    getEmbedCode: function(gallery) {
        var attribution = 'Gallery from ' + gallery.posts[0].location.address,  
            file = window.mode == 'production' ? 'gallery.js' : 'gallery-dev.js';

        var embed = '<blockquote class="fresco-media" ';
        embed += 'data-gallery="' + gallery._id + '" ';
        embed += 'data-start="0" ';
        embed += 'data-width="100%" ';
        embed += 'data-cycle="false">';
        embed += '<p>' + gallery.caption + '</p>'
        embed += '&mdash; '+ attribution + '. ';
        embed += '<a href="' + this.baseUrl() + '/gallery/' + gallery._id + '">' + moment(gallery.time_created).format('MMMM Do, YYYY') + '</a>';
        embed += '</blockquote>';
        
        embed += '<script src="' + this.CDN + '/embed/' + file +'" type="text/javascript" async></script>';
