...
const utils = {
    ...
    isOriginalGallery({ id = '', posts = [] }) {
        return posts && posts.every(p => p.parent_id === id);
    },

    isImportedGallery({ owner_id = null, posts = [] }) {
        return !owner_id && (posts && posts.every(p => !p.owner_id));
    },

    isSubmittedGallery({ owner_id = null, posts = [] }) {
        return owner_id && (posts && posts.every(p => p.owner_id === owner_id));
    },
};
