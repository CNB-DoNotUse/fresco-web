...
const utils = {
     ...
     */
    getBylineFromGallery(gallery) {
        if (!gallery) return null;

        if (gallery.external_account_name) return gallery.external_account_name;
        else if (gallery.owner) {
            if (gallery.owner.full_name) return gallery.owner.full_name;
            else if (gallery.owner.username) return gallery.owner.username;
        }

        return null;
    },

    getBylineFromPost(post) {
        if (!post) return null;

        if (post.parent && post.parent.external_account_name) {
            return post.parent.external_account_name;
        } else if (post.owner) {
            if (post.owner.full_name) return post.owner.full_name;
            else if (post.owner.username) return post.owner.username;
        }
    ...
    getAvgFromMultipoint(loc) {
        let lng;
        let lat;
        if (!loc || !loc.type === 'multipoint' || !loc.coordinates) return null;
        if (loc.coordinates.length && loc.coordinates.length > 1) {
            lng = location.coordinates.reduce((p, c) => p[0] + c[0], 0)
                / location.coordinates.length;
            lat = location.coordinates.reduce((p, c) => p[1] + c[1], 0)
                / location.coordinates.length;
        } else {
