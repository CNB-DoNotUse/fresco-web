...
const utils = {
    ...
    /**
     * Returns a byline for a post, takes a post as the first parameter
     * @param  {Object}  post Post to generate from
     * @param  {Boolean} prefix Whether to prefix the byline or
     * @return {String} A byline for the post
     */
    getBylineFromPost({
        parent = {
            external_account_name: '',
        ...
        if (owner && owner.full_name) 
            return `${prefix ? 'Post by ' : ''}${owner.full_name}`;
        else if (owner && owner.username) 
            return `${prefix ? 'Post by ' : ''}${owner.username}`;
        else if (parent && parent.external_account_name) {
            if (parent.external_source) 
                return `${parent.external_account_name} via ${parent.external_source}`;
            else
                return `Post by ${parent.external_account_name}`;
        } 
        else if (curator && curator.full_name) 
            return `${prefix ? 'Imported by ' : ''}${curator.full_name}`;
    ...
    getAvgFromMultipoint(loc) {
        ...
        const length = loc.coordinates.length;
        if (length > 1) {
            lng = loc.coordinates.reduce((acc, cur) => acc + cur[0], 0) / length;
            lat = loc.coordinates.reduce((acc, cur) => acc + cur[1], 0) / length;
        } else {
