...
const utils = {
    ...
    getBylineFromGallery(gallery) {
        const { external_account_name, owner: { full_name, username } } = gallery;

        return [full_name, username, external_account_name].find(s => (
            typeof(s) === 'string' && s.length > 0
        ));
    },

    getBylineFromPost(post) {
        const { parent: { external_account_name }, owner: { full_name, username } } = post;
