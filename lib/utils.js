...
const utils = {
    ...
    getBylineFromGallery({
        ...
        if (external_account_name) {
            if (external_source) 
                return `${external_account_name} via ${external_source}`;
            return external_account_name;
        } else if (owner && owner.full_name) {
    ...
    /**
     * Returns the correct byline from a post's attributes
     * @param  {Object} post The post, sets parent and owner by default
     * @return {String} A string value for the byline
     */
    getBylineFromPost({
        parent = { external_account_name: '', external_source: '' },
        owner = { full_name: '', username: ''}
    }) {
        //Externally imported, not a submission
        if (parent && parent.external_account_name) {
            if (parent.external_source) 
                return `${parent.external_account_name} / ${parent.external_source}`;
            return parent.external_account_name;
        } else if(owner) {
            return owner.full_name || owner.username
        }
