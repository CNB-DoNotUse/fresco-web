...
const utils = {
    ...
    formatImg(img, size) {
        ...
        if (formatted.indexOf('cdn.dev') !== -1) {
            formatted = formatted.replace('photo/', `photo/${size}`);
        ...
        if (!size || size === 'original') return formatted;
        if (formatted.indexOf('d2j1l98c0ybckw.cloudfront.net') === -1) return formatted;
     ...
     */
    mergeAssignments(assignments, fieldToSortBy) {
        return assignments.global.concat(assignments.nearby).sort((a1,a2) => {
