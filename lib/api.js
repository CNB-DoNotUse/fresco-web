    ...
    request: (options) => {
        return new Promise((resolve, reject) => {
            const end = (error, response) => {
                if(error) {
                    // Check if API doesn't give a status code, and no proper response
                    if(!has(error, 'response.body.error.status')) {
                        if(has(error, 'response.body.error.stack')){
                            reject({type: error.response.body.error.stack, status: error.status});
                        } else {
                            reject({type: 'Failed to connect to the API', status: error.status});
                        }
                    } else {
