    ...
    request: (options, req = null, retrying = false) => {
        return new Promise((resolve, reject) => {
            if (options.req) {
                ...
                if(!options.token && options.req.session.user !== null && options.req.session.token.token !== ''){
            ...
            if(options.token === '' || typeof(options.token) == 'undefined') {
                request.set('Authorization', auth.basicAuthentication(config.API_CLIENT_ID, config.API_CLIENT_SECRET));
            } else {
                request.set('Authorization', auth.bearerAuthentication(options.token));
            }
