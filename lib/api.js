...
var API = {
  ...
  request: (options, callback) => {
    ...
    var message = request(options.method || 'GET', config.API_URL + '/' + config.API_VERSION + options.url)
    ...
    if (options.method == 'POST' && Object.keys(options.files || {}).length > 0) {
      ...
      for (index in options.files) {
        ...
        message.attach(options.files[index].fieldname, options.files[index].path);
      }
