...
var API = {
    ...
    */
    proxy: (req, res, callback) => {
        ...
        //Send request
        API.request({req, res}).then(response => callback(response.body)).catch(err => res.status(err.status).send(err));
    },
    ...
    */
    request: (options) => {
        return new Promise((resolve, reject) => {
            ...
            var request = superagent(options.method ||
                                     'GET', config.API_URL + '/' + config.API_VERSION + options.url);
