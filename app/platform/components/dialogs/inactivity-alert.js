        ...
        .then((res) => {
            if (res.length > 0) {
                if (time.past12Hours(res[0].created_at) && !checkIfNotified()) {
                    debugger
                    $.snackbar({content: "You haven't made an assignment in 12+ hours. Fresco works best with momentum. Expect quality responses after three consecutive days of posting assignments consistently."})
                    markNotified();
                }
            }
        })
