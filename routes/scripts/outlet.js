...
router.get('/outlet/export', (req, res) => {
  ...
  API.proxy(req, res, (body) => {
    ...
    if(req.query.format == 'xlsx'){
      var data = [['time', 'type', 'price', 'assignment title', 'assignment ID', 'outlet', 'user', 'user ID', 'post']];

      lines.forEach(function(line){
	var x = new Date(line.time),
	      formattedTime = (x.getMonth() + 1) + '/' + x.getDate() + '/' + x.getFullYear();
        data.push([formattedTime, line.type, line.price.replace('$', ''), line.assignment, line.assignmentId, line.outlet, line.user, line.user_id, line.post]);
      });
      ...
      var output = "time,type,price,assignment title, assignment ID,outlet,user,userId,post\r\n";

      lines.forEach(function(line){
        var x = new Date(line.time),
        formattedTime = (x.getMonth() + 1) + '/' + x.getDate() + '/' + x.getFullYear();
        output += formattedTime + ',' + line.type + ',' + line.price.replace('$', '') + ',' + line.assignment + ',' + line.assignmentId + ',' + line.outlet + ',' + line.user + ',' + line.user_id + ',' + line.post + '\r\n';
      });
