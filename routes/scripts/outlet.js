...
router.get('/outlet/export', (req, res) => {
  ...
  API.proxy(req, res, (body) => {
	...
	var x = new Date(line.time),
        ...
        data.push([formattedTime, line.type, line.price.replace('$', ''), line.assignment, line.outlet]);
      });
      ...
      lines.forEach(function(line){
        var x = new Date(line.time),
        formattedTime = x.getMonth() + '/' + x.getDate() + '/' + x.getFullYear();
        output += formattedTime + ',' + line.type + ',' + line.price.replace('$', '') + ',' + line.assignment + ',' + line.outlet + '\r\n';
      });
